...
| 파일 | 방향 | 소유 |
|---|---|---|
| `risk_assessment.schema.json` | ① 예측 AI → ② 판단 엔진 | AI 담당 |
| `action_decision.schema.json` | ② 판단 엔진 → ③ 경로 엔진 | 백엔드 |
| `safe_route.schema.json` | ③ 경로 엔진 → ② 판단 엔진 | 경로 담당 |
| `assess_response.schema.json` | 통합 API → ④ UI | 백엔드 |
| `official_info.schema.json` | 픽스처 → ② | 기획 PM |

**스키마 파일 자체는 계약 오너 1명만 고친다.** 다른 사람은 요청한다.
...
| 필드 | 상태 | 언제 정리하나 |
|---|---|---|
| `area_risk.score` | `DEPRECATED`. `risk_probability` 의 예전 이름 | 픽스처 생성기 갱신 후 (O-14) |
