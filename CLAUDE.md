...
> **설계·회의·감사 문서는 이 저장소에 없다.** 공개 저장소라 내부 문서를 두지 않기로
> 했고(v1.0.0), `docs/` 와 `0811_공유용.txt` 를 내렸다. 아래에서 문서 이름을 그대로
> 부르는 것은 **팀이 따로 관리하는 그 문서**를 가리킨다 — 판단의 근거가 무엇이었는지는
> 남겨야 다음 사람이 되짚을 수 있기 때문이다. 파일이 필요하면 저장소 소유자에게 받는다.
>
> 지운 것은 저장소의 사본일 뿐이며 **git 히스토리에는 남아 있다.**

충돌하면 위에서부터 이긴다.

1. **현재 재현 가능한 데이터 산출물과 `contracts/schema/`의 실제 스키마**
2. `최종 회의 확정사항` · `확인완료 팀원 답변`
   — 팀 전원이 확정한 것이며 `DECISIONS.md` 2.3에 `M-<안건번호>` ID로 정리돼 있다
3. `요구사항 정의서` · `MVP 설계서`
4. `문서 정합성 평가서`의 최종 결정
...
- 현재 상태는 `REPOSITORY_AUDIT.md` **6절과 10절**이 정본이다.
  **`마른길_MVP_설계서.md` 8.5.4 와 `HACKATHON_11H_RUNBOOK.md` 의 T+ 일정표는
...
금칙어 검사와 대칭이다. **쓰면 안 되는 말**은 위 표가 막고, **빠지면 안 되는 말**은
`web/src/map/features.test.ts` 가 지킨다. 지도 도형은 `MapFeature.limit` 을 필수
필드로 들고 다니므로 한계 문구 없이 도형을 만들 수 없다.

이 검사를 만든 이유가 있다. 선 스타일만 바꾼 커밋이 RT-02 한계 문구
(`추천 후보 경로 (직선 표시 · 실제 경로 아님)`)를 지웠는데, 그 문구는 leaflet 팝업
문자열이라 jsdom 에 보이지 않아 **어떤 검사도 빨개지지 않았다.** 사람이 눈으로
발견해 되살렸다(`dba9648`). 같은 파일이 **좌표를 지어내지 않는지**도 본다 —
모든 도형의 좌표가 응답에 있던 값인지 대조한다.
...
`api`와 `check`는 출력을 `logs/`에 남긴다. 장애 대응은 `OPERATIONS.md`.
...
- **분 단위 급등 감시** — 급변 감시규칙을 붙이지 않기로 확정했다(M-06). `분단위 해상도 확장계획` 참조
...
| 알고 싶은 것 | 파일 |
|---|---|
| 무엇을 정했고 무엇을 아직 안 정했나 | `DECISIONS.md` — 회의 확정은 2.3, OPEN은 3절 |
| 지금 실제로 구현된 것 | `REPOSITORY_AUDIT.md` 6절 |
...
| 장애가 나면 누가 무엇을 하나 | `OPERATIONS.md` |
| 발표에서 무엇을 숨기지 말아야 하나 | `HACKATHON_CHECKLIST.md` |
