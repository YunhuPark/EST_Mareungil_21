...
이 파일이 지금도 필요한 이유
----------------------------
2026-08-23 부터 픽스처는 **생성기가 엔진을 불러** 만든다(`services/pipeline`). 그래서
"픽스처와 엔진이 다를 수 있는가"라는 질문은 `test_fixture_engine_roundtrip.py` 가
응답 전체로 답한다 — 그쪽이 더 넓다.

그럼에도 이 파일을 남기는 이유는 **대조하는 각도가 다르기** 때문이다. 라운드트립은
"파이프라인을 다시 돌리면 같은 파일이 나오는가"를 보고, 이 파일은 필드 하나하나가
**어느 함수에서 나와야 하는지**를 이름으로 적어 둔다. 파이프라인 전체가 함께 틀리면
라운드트립은 초록으로 남지만 이 파일은 `decide()`·`apply()` 를 직접 불러 대조하므로
잡힌다.
