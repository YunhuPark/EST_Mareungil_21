...
| 대피시설 | 원자료 107개 → **후보 7곳** | `EVACUATE` 안전거점 후보 | **후보 집합 확정** (C-32, `contracts/safe_points.json`). 상태 태그는 미연동 |
...
**범위 조건은 확정됐다 (C-32).** 경로 범위(강남역 반경 1km)를 적용해 후보를 **7곳**으로 닫았고 정본은 `contracts/safe_points.json`이다. 범위를 새로 정한 것이 아니라 RT-15·O-09에서 이미 닫힌 값을 그대로 쓴 것이다. 7곳은 전부 학교·관공서라 M-25(민간건물 제외)가 별도 규칙 없이 충족된다. **상태·프로필 조건과 후보 순위는 아직 없다** — 시설 상태는 여전히 고정 픽스처(`DS-S7`·`DS-S8`)이고 선택 로직은 미구현이다.
