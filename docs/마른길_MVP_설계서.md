...
**OPEN(O-15).** `DANGER`의 "추가 위험신호" 집합은 아직 확정 전이다. 현재 셀 수 있는 것은 TH-02(60분 누적 강우 40mm) 하나이며, TH-01(10분 강우)은 기준값은 확정돼 있으나 계약이 값을 나르지 않아 쓸 수 없고, 하수 부담은 이미 `ai_risk_level`에 들어와 있어 다시 세지 않는다. 확인 결과는 [DECISIONS.md](./DECISIONS.md) 3.4에 있다.
