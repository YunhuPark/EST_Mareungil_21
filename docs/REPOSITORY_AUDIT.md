...
**이것은 코드 결함이 아니라 미확정 안전정책이다.** 둘 중 하나를 정해야 하며 임의로
정하지 않는다 — `DECISIONS.md` 3.1 의 **O-16** 으로 열었다.

> **`observed_rate` 에는 스키마 `description` 이 없다.** DQ-03 임계가 재는 대상이
> 계약에 정의돼 있지 않다는 뜻이다. 무엇의 비율인지부터 적어야 판단할 수 있다.
