...
> **스키마 5개 · 픽스처 20건 · Python 179건(8개 모듈) · 프론트 15건**이었다.
>
...
> 상호참조 18곳을 옮겼다.
>
> **갱신 2026-08-17 오프라인 해커톤 직전 실사.** 위 “픽스처 20건 · Python 179건 · 프론트
> 15건”은 그 시점의 값이며 **지금은 다르다.** 실제로 돌려 확인한 현재 검증 규모는
> **스키마 5개 · 픽스처 23건(유효 12 + 거부 11) · Python 208건(9개 모듈) · 프론트 25건**이고
> `tsc --noEmit` 과 production build 도 통과한다. 같은 실사에서 새로 찾은 것은 **10절**에 적었다.
>
...
| 계약 검증 명령 | **구현됨** | `.\make.ps1 contracts` — 픽스처 23건 (유효 12 + 거부 예제 11) |
...
| enum 3중 동기화 검사 | **구현됨** | `tests/test_enum_sync.py` 18건. 핵심 9축 3중 일치 + **필드 이름으로 묶은 스키마 사본 교차 검사** + **TS 라벨 표 커버리지**. 뒤의 둘은 검사 보강에서 추가했다 — 그 전에는 `closures.kind` 사본을 어긋나게 두거나 `VERIFICATION_LABEL` 에서 `DEMO_FIXTURE` 를 지워도 전부 통과했다 |
...
| **픽스처 STUB ↔ `classify()` 대조 (C-28)** | **구현됨** | `tests/test_demo_fixture_consistency.py` 12건. **등급 축만 본다** — 행동 축은 판정 본체가 없어 검사할 대상이 없다. 원본 데이터를 필요로 하지 않는다 |
| **관측률 지표 (O-16 → C-28)** | **구현됨** | 계산 정본은 `scripts/mareungil/sewer.py` `observed_rate()` 하나이며 `build_demo_fixtures.py`(전체 재생성)와 `refresh_observed_rate.py`(재학습 없는 갱신)가 같이 쓴다. **임계 0.70 은 여전히 `TEAM_AGREED`** |
...
| **서비스 위험 등급 (C-23·C-27)** | **구현됨** | `classify()`. O-15 확정으로 `DANGER` 추가 신호는 TH-02 하나. 미확정 항목 없음 |
| **판단 엔진 (우선순위 1~10)** | **없음** | T+3:00~6:00. 규칙 9 가 쓸 `rain_past_10m_mm` 은 픽스처에 **실려 있으나 읽는 코드가 없다** |
| **경로 엔진 (후보 30개 비교)** | **STUB** | 인터페이스만. T+6:00~8:00 |
| **`DS-S6`** | **있음** | M-16 목적지 차단. risk 는 실제 모델 출력(RF-S2), 통제는 **시연용 합성값**(`DEMO_FIXTURE`) |
| **`DS-S2` ~ `DS-S5`** | **없음** | T+6:00~8:00 |
| **공식정보 픽스처 값** | **구현됨** | O-11 확정. `VERIFIED_SOURCE`, 원문 대조표 30건. 경보 4 · 통제 6 · 확인 침수 7 |
| **재생 시각 공식정보 필터 (M-36)** | **구현됨** | `visible_at()` 을 생성기가 호출한다. 21:40 에 공개돼 있던 공식 통제는 **0건**이며 그것이 사실이다 |
...
| 위험 | 영향 | 대응 |
|---|---|---|
| ~~`official_0808.json` 이 비어 있다~~ | — | **닫혔다**(O-11 → C-25). `VERIFIED_SOURCE` 이며 원출처 대조표 30건이 들어 있다. `DS-S6` 도 작성됐다 |
...
## 9. 정합성 점검에서 새로 발견한 것 (2026-08-16 · 2026-08-17 추가)

`.\make.ps1 check` 가 **전부 통과하는 상태에서** 문서·코드·계약을 대조해 찾은 것들이다.
**테스트가 통과한다는 것과 테스트가 무언가를 지킨다는 것은 다르다** — 아래가 전부 그 차이에서 나왔다.
...
| `DS-S1`·`DS-S6` 의 `service_risk_level` | 계약 검증·`test_service_risk.py` 모두 통과 | **픽스처가 `SAFE` 인데 같은 입력으로 `classify()` 를 돌리면 `CAUTION` 이었다.** `observed_rate` 가 DQ-03 임계 아래라 `quality_low` 가 켜진다. 계약 검증은 스키마 모양만 보고, 등급 테스트는 손으로 만든 입력으로 `classify()` 를 부른다 — **픽스처가 그 함수를 통과한 값인지 아무도 대조하지 않았다.** `DS-S7`·`DS-S8` 은 등급이 우연히 맞아(`DANGER`) 사유에서 `DATA_QUALITY_LOW` 만 빠져 있었다 | C-28 · `tests/test_demo_fixture_consistency.py` |
...
## 10. 오프라인 해커톤 직전 실사에서 새로 발견한 것 (2026-08-17)

`.\make.ps1 check` 가 **전부 통과하는 상태에서** 찾은 것들이다. 9절과 같은 종류다 —
**통과한다는 것과 무언가를 지킨다는 것은 다르다.**

### 10.1 `classify()` 와 데모 픽스처의 등급이 어긋난다

구현된 `classify()` 에 `DS-*` 픽스처의 입력을 그대로 넣으면 **두 건이 불일치한다.**

| 시나리오 | 픽스처 `service_risk_level` | `classify()` 결과 | `observed_rate` |
|---|---|---|---|
| `DS-S1` | `SAFE` | **`CAUTION`** | 0.562 |
| `DS-S6` | `SAFE` | **`CAUTION`** | 0.562 |
| `DS-S7` | `DANGER` | `DANGER` | 0.548 |
| `DS-S8` | `DANGER` | `DANGER` | 0.548 |

원인은 DQ-03 이다. `service_risk.OBSERVED_RATE_MIN` 은 `0.70` 인데 **RF 픽스처 4개가 전부
그 아래다**(0.548~0.562). `classify()` 는 이것을 `quality_low` 로 보고 `degraded` 를 참으로
만들어 등급 하한을 `CAUTION` 으로 올린다. 반면 생성기는
`build_demo_assess_fixtures.py:189`·`:607` 에서 `"service_risk_level": "SAFE"` 를 **하드코딩**한다.

**왜 아무도 못 잡았나 — 세 겹이다.**

- `api/main.py` 가 `classify()` 를 호출하지 않는다 (10.2 참조)
- 픽스처 생성기도 호출하지 않는다. 등급을 손으로 적는다
- `tests/test_service_risk.py` 는 `observed_rate=1.0` 같은 **합성 입력**으로만 시험한다.
  실제 데모에서 나오는 0.548 을 한 번도 넣어보지 않았다

**파급.** 규칙 9 가 "품질 하향 → `WAIT`" 이므로, DQ-03 을 그대로 적용하면 이 재생
데이터에서는 **`SAFE` 도 `MOVE` 도 나올 수 없다.** 데모 첫 화면이 `SAFE`+`MOVE` 가 아니라
`CAUTION`+`WAIT` 이 된다.

**이것은 코드 결함이 아니라 미확정 안전정책이었다.** `DECISIONS.md` 3.1 의 **O-16** 으로
열었고, **같은 날 팀장 승인으로 닫혔다 — 2절 C-28.**

> **닫힌 방식이 이 절의 결론을 바꾼다.** 임계 `0.70` 을 손대지 않고 `observed_rate` 의
> **계산식**을 연속 지표로 바꿨다. 원인이 임계가 아니라 지표였기 때문이다 —
> `sample_count >= 10` 컷오프가 분포의 중앙값(10) 바로 위에 놓여 판독 하나 빠진 센서가
> 통째로 미관측이 됐다. 네 시점이 **0.774~0.956** 으로 올라가 임계를 통과하고
> **픽스처 등급이 그대로 코드와 일치한다.** 위 표의 불일치는 더 이상 재현되지 않으며,
> `tests/test_demo_fixture_consistency.py` 12건이 이 대조를 고정한다.

### 10.2 구현된 판단 모듈 셋을 런타임에 아무도 호출하지 않는다

`classify()`·`apply()`·`visible_at()` 은 구현돼 있고 테스트 53건이 붙어 있다. 그런데
`api/main.py` 가 `services` 에서 import 하는 것은 **`Profile` enum 하나뿐**이다.

| 함수 | 테스트 | 런타임 호출부 |
|---|---|---|
| `service_risk.classify()` | 27건 | **없음** |
| `postprocess.apply()` | 14건 | **없음** |
| `official.visible_at()` | 12건 | 픽스처 **생성 시**에만 (`build_demo_assess_fixtures.py`) |

**"결정 엔진이 절반 구현됐다"의 정확한 뜻은 로직은 있고 배선이 없다이다.** 이 구분이
...
`HACKATHON_11H_RUNBOOK.md` F-3 과 `HACKATHON_CHECKLIST.md` 가 둘 다 필수로 건
production 리허설(`npm run build` + `npm run preview`)이 **현재 설정으로는 실패한다.**

- `web/vite.config.ts` 의 `/api` 프록시는 `server` 블록에만 있고 `preview` 블록이 없다
- `web/src/api.ts:10` 은 `BASE` 를 빈 문자열로 두므로 preview(4173)는 자기 자신에게 요청한다
- `VITE_API_BASE` 를 박아 빌드해도 `api/main.py` 의 CORS 가 `:5173` 두 개만 허용한다
- `make.ps1`·`make.sh` 에 `preview` 태스크가 아예 없다

**체크리스트의 장애 대응 "프론트가 하얗다 → `preview` 빌드본으로 전환"은 지금 작동하지
않는 fallback 이다.**

## 11. 픽스처와 엔진의 정본이 둘이었다 (2026-08-23 · 닫힘)

10.1~10.3 과 같은 종류다 — **`.\make.ps1 check` 가 전부 통과하는 상태에서** 찾았다.

### 11.1 무엇이 어긋났나

`scripts/build_demo_assess_fixtures.py` 는 `decision`·`route` 를 **손으로 적었고**,
`api/main.py` 는 응답을 조립할 때 그 두 블록을 **통째로 덮어썼다.** 같은 사실에 대한
정본이 둘이었고, 사용자에게 가는 것은 언제나 뒤쪽이었다.

| | 픽스처에 적혀 있던 것 | 화면에 나간 것 |
|---|---|---|
| `DS-S1` 근거 | 3줄 (`AI_AREA_LOW` 지역값 0.0 · `RAIN_BELOW_THRESHOLD` · `NO_OFFICIAL_ORDER`) | **1줄** (`NO_TRIGGER` "행동을 바꿀 조건이 확인되지 않았습니다.") |
| `DS-S1` 경로 | 후보 2건 · `eta_sec` 720 · `detour_ratio` 1.0 | 후보 없음 · `eta_sec` null |
| `DS-S1` `source_kind` | `FIXTURE` | `LIVE_PIPELINE` |
| `DS-S7` 근거 | 2줄 | 1줄 |

**아무도 못 잡은 이유.** 프론트 테스트가 픽스처를 `initialData` 로 직접 렌더링한다.
즉 화면 테스트가 **API 가 내보내지 않는 형태**를 검증하고 있었다 — C-21 이 지적한
"픽스처를 정제해서 넣어 그대로 받는다를 증명하지 못했다"와 같은 실수가 다른 자리에서
반복된 것이다. 실제로 배지 검사 두 건이 `DS-S1` 에 `FIXTURE` 배지를 기대하고 있었는데,
API 는 그 시나리오를 이미 `LIVE_PIPELINE` 로 내보내고 있었다.

### 11.2 닫은 방식

조립을 **한 곳으로 모았다.** `services/pipeline.apply_engine()` 이 유일한 조립 지점이고
`api/main.py` 와 픽스처 생성기가 같은 함수를 부른다. 생성기는 이제 행동·등급·근거·경로를
손으로 적지 않는다.

새 모듈이 필요했던 이유는 둘 곳이 없었기 때문이다 — `services/decision` 은
`services/route` 를 import 할 수 없고(CLAUDE.md 10절), `scripts/` 는 `api/` 를 import 할
수 없다. 두 규칙을 지키려면 둘 위에 있는 모듈이 있어야 한다.

**엔진이 재현하지 못하는 것은 그대로 남겼다.** `DS-S7`·`DS-S8` 의 `route` 는 시설
만석·폐쇄 서사이고 저장소에 대피시설 운영상태 원자료가 없다(M-32). 그 둘만
`source_kind: FIXTURE` 이며 `_stub` 이 사실을 표시한다.

### 11.3 무엇이 이것을 다시 막나

`tests/test_fixture_engine_roundtrip.py` — 픽스처를 파이프라인에 통과시키면 **자기 자신이
나와야 한다.** 응답 전체를 대조하므로 필드를 더해도 자동으로 덮는다.

**계약 검증으로는 못 잡는다.** 실제로 확인했다. `DS-S1` 의 `reason_code` 를
`NO_TRIGGER` -> `AI_AREA_LOW` 로 손으로 고치면 계약 검증은 `ok` 를 내고 라운드트립만
실패한다. 계약이 보는 것은 **형식**이고 이 검사가 보는 것은 **출처**다.

### 11.4 남은 것 — 근거 문구는 엔진이 더 가난하다

**이 절이 닫은 것은 정본이 둘이었다는 구조 문제이지 문구 품질이 아니다.** 팀이 쓴
`AI_AREA_LOW` 3줄 근거가 `NO_TRIGGER` 1줄보다 훨씬 낫고, 화면은 계속 1줄만 받는다.
바꾸려면 `decide()` 가 내는 `reasons` 를 늘려야 하는데 그것은 화면 문구를 정하는
일이라 이 작업 범위 밖이다(CLAUDE.md 9절). **`OPEN:` 항목으로 남긴다.**
